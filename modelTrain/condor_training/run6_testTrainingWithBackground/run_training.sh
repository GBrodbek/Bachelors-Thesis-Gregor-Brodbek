...
(
    ...
    xrdcp -r root://ceph-node-j.etp.kit.edu://gbrodbek/newTrainingFiles/output_1149668_{0..9}.root tmp/
...
#  from the beginning
python3 -m src.train_lightning1 --data-train ${HOME}/tmp/output_1149668_{0..9}.root ${HOME}/tmp/background_1160162_{0..9}.root --data-config config_files/config_hit_tracks_tau.yaml -clust -clust_dim 3 --network-config src/models/wrapper/example_mode_gatr_e.py --model-prefix ${HOME}/tmp/modelsaves/ --num-workers 0 --gpus 1 --batch-size 25 --start-lr 1e-3 --num-epochs 10  --fetch-step 0.1 --log-wandb --wandb-displayname run5_newWithBackground_10fileseach --wandb-projectname topas_logs --wandb-entity gbrodbek-kit4749
