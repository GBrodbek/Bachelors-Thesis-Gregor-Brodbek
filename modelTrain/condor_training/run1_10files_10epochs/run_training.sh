...
(
    ...
    xrdcp -r root://ceph-node-j.etp.kit.edu://gbrodbek/trainingFiles/output_1279601_{40..50}.root tmp/
)
...
# Run training
cd PID_GNN
python3 -m src.train_lightning1 --data-train ${HOME}/tmp/output_1279601_{40..50}.root  --data-config config_files/config_hit_tracks_tau.yaml -clust -clust_dim 3 --network-config src/models/wrapper/example_mode_gatr_e.py --model-prefix ${HOME}/tmp/modelsaves/ --num-workers 1 --gpus 1 --batch-size 16 --start-lr 1e-3 --num-epochs 10  --fetch-step 0.1 --log-wandb --wandb-displayname files10epochs10 --wandb-projectname topas_logs --wandb-entity gbrodbek-kit4749